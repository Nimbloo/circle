...
import { decideJoinRequest } from '@/lib/api/teams';
 ...
 * Devolve `{ requests, members }` (fila pendente + membros atuais do time).
 */
export async function POST(req: Request, { params }: Params) {
   return handle(async () => {
      ...
      return ok(await decideJoinRequest(db, teamKey, id, decision, me.id));
   }, req);
}
