...
const { default: WebhooksSettings } = await import('@/components/common/settings/webhooks-settings');
...
describe('webhooks: rollback só do item (Ad#21–40)', () => {
   it('excluir que falha devolve só o webhook excluído, sem desfazer o toggle de outro', async () => {
      ...
      expect(
         screen.getByRole('switch', { name: 'Ativar https://x.test/b' }).getAttribute('aria-checked')
      ).toBe('false');
   });
});
